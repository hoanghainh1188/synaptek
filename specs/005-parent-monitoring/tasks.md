...
- [x] T001 Tạo migration `supabase/migrations/0005_m4_parent.sql` theo `contracts/db-schema-0005.md`
...
- [x] T002 Trigger `handle_new_user`: mở rộng nhận `role='parent'` từ metadata đăng ký (hiện chỉ teacher/student).
- [x] T003 Áp local: `supabase db reset` (0001–0005 sạch); kiểm bảng/policy/hàm tồn tại.
- [x] T004 [P] Bộ test RLS `supabase/tests/rls-0005.sql` theo `contracts §7` (PH đọc con mình; chặn HS/PH khác;
...
- [x] T005 [US1] `apps/app/src/lib/supabase/parent.ts`: HS tạo/thu hồi `parent_link_code` (qua
...
- [x] T006 [US1] AuthForm + auth.signUp: thêm lựa chọn vai trò **Phụ huynh** (student/teacher/parent).
- [x] T007 [US1] `profile.tsx` (HS): khối "Mã liên kết phụ huynh" — tạo/hiển thị/thu hồi mã; danh sách PH
      đang theo dõi + gỡ. Đổi vai trò thêm 'parent'.
- [x] T008 [US1] Route PH `apps/app/src/app/(parent)/children.tsx`: nhập mã liên kết + danh sách "Con của
      tôi" + gỡ. Guard `role=parent`.
- [x] T009 [US1] Trang chủ + profile: lối vào theo vai trò PH ("Con của tôi" → /children).
- [x] T010 [US1] E2E `apps/app/tests/e2e/parent-link.spec.ts` (auth-gated): HS tạo mã → PH nhập → con hiện;
...
- [x] T011 [US2] `lib/supabase/parent.ts` (tiếp): đọc tiến độ 1 con (RLS-gated): `skill_mastery`→mastery map,
      `gamification_state` (XP/streak), đếm `attempts`, `submissions` (điểm cuối qua `displayScore`).
- [x] T012 [US2] Route PH `apps/app/src/app/(parent)/child/[id].tsx`: bảng theo dõi read-only — điểm yếu
      (`@synaptek/learning-path` skillWeakness) + XP/streak + tổng quan luyện tập + kết quả bài. Trạng thái rỗng.
- [x] T013 [US2] Từ "Con của tôi" → chạm con → mở bảng theo dõi.
- [x] T014 [US2] E2E `apps/app/tests/e2e/parent-monitor.spec.ts` (auth-gated): con có dữ liệu → PH thấy
...
- [x] T015 [P] `npm run format` + `npm test` toàn workspace xanh (regression engine — FR-012).
- [x] T016 [P] RLS M3 (`rls-0003.sql`) + M4 (`rls-0005.sql`) chạy local xanh; `expo export` web xanh.
- [x] T017 [P] Trạng thái rỗng + edge: con chưa có dữ liệu, mã sai/thu hồi, guest/role khác chặn route PH.
- [x] T018 Cập nhật `docs/WORKING-NOTES.md` + `docs/02-roadmap.md` (M4 trạng thái) + `docs/FEATURE-MAP.md`
