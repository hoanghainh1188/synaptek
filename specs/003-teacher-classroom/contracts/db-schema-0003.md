...
-- submissions: HS chỉ ĐỌC của mình; GV lớp ĐỌC + GHI ĐÈ điểm/nhận xét. HS KHÔNG ghi trực tiếp —
-- nộp bài đi qua Edge `grade-assignment` (service_role) ghi answers + auto_score (D4).
...
> **Quan trọng (đã verify qua RLS test)**: KHÔNG grant table-level UPDATE cho `authenticated` — grant
> table-level che mất `revoke` cấp cột nên HS vẫn đặt được `auto_score`. Thay vào đó: HS **không** có
> insert/update trực tiếp; chỉ **grant UPDATE theo cột** `(final_score, feedback, is_override)` cho GV
> ghi đè (kết hợp policy `submissions_teacher_update`). `auto_score`/`answers` chỉ **service_role** ghi
> (Edge). HS nộp bài → gọi Edge `grade-assignment` (service_role upsert).
...
-- submissions: HS chỉ SELECT; GV ghi đè CHỈ theo cột (KHÔNG table-level UPDATE — xem cảnh báo §5):
grant select on public.submissions to authenticated;
grant update (final_score, feedback, is_override) on public.submissions to authenticated;
-- service_role cho Edge grade-assignment (ghi answers + auto_score xuyên HS):
