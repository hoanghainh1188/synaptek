...
| **D13** | **Edge Function import engine theo TÊN qua import map** (`supabase/functions/deno.json` → `../../packages/grading-engine/src`), không vendor/copy                                                                        | Giữ một nguồn-sự-thật cho engine (D5); function chấm chính thức nhận `{questionId, answer}`, tra đáp án ở server (đáp án không rời server). Rủi ro: bundling file ngoài `functions/` khi deploy → fallback publish JSR/npm hoặc vendor `_shared/` | Scaffolded — cần verify với Supabase CLI               |
