...
| **Tương lai**                         | THCS/THPT sâu hơn; môn khác đầy đủ (Lý/Hóa/Anh); LaTeX-as-input (gõ trực tiếp); gia sư AI mở rộng (chat tự do)                                                                                                                                     | —                                                                                                | 🟡 **đã khởi động** — khung cấp học 2 tầng (D54) + **pilot THCS Toán lớp 6 Số nguyên** (D55, 17 câu, quy trình AI-nháp→chủ-repo-duyệt) đã ship; chấm trình bày từng bước (D38–D45) + LaTeX RENDER (D53) cũng đã ship. Còn: nhân rộng chủ đề THCS/THPT · môn khác (Tiếng Anh cần bộ chấm khác moat) · LaTeX-as-input · gia sư AI chat tự do — xem "Sau M4" |
...
`docs/00-architecture.md` §0 D26–D55 và `CLAUDE.md` mục "Recent Changes"):
...
- **Đa môn + cấp học**: nền `Grade.subject` + seed Tiếng Việt lớp 1–3 (D39); khung cấp học 2 tầng
  Tiểu học/THCS/THPT (D54); **pilot THCS Toán lớp 6 Số nguyên** (D55, 17 câu — nội dung cấp trên đầu tiên,
  quy trình AI-nháp→chủ-repo-duyệt); gate tự-chấm nội dung (`validate:content` chạy engine trên đáp án).
  Nội dung tổng 222 câu.
