...
// Engine đã parse: số kiểu VN (phẩy), phân số a/b, mũ ^, ngoặc (), biến x, +−*/, căn √ (D8/D9/D45).
export type KeypadType = "numeric" | "fraction" | "expression";

const DIGITS = ["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"];

/** Phím hiển thị cho từng loại — numeric gọn (+ căn); fraction thêm '/'; expression thêm mũ/ngoặc/biến/toán tử/căn. */
export function mathKeypadKeys(qtype: KeypadType): string[] {
  ...
  return [...DIGITS, "/", ",", "-", "√"]; // numeric (giữ '/' + dấu âm, thêm căn — D45)
}
